...
void main(List<String> args) {
  if (args.isEmpty) {
    stderr.writeln(
      'Usage: dart run bin/xauusd_batch3_robustness.dart <history-directory>',
    );
    exitCode = 64;
    return;
  }
  ...
  final selected = Batch3ResearchSelection.selectHistoricalPass(batch);

  stdout.writeln(
    'Batch-3 first-gate hypotheses: ${selected.length}/'
    '${Batch3ResearchSelection.historicalPassIds.length}.',
  );
  stdout.writeln(
  ...
  // Build overlap against the full historical-pass mainstream pool. This is
  // diagnostic only: overlap does not decide the segmentation gate.
  final allSegments = const StrategyEdgeSegmentation().evaluate(batch);
  final segments = allSegments
      .where(
        (segment) => Batch3ResearchSelection.historicalPassIds.contains(
          segment.strategyId,
        ),
      )
      .toList(growable: false);

  stdout.writeln('\n=== BATCH-3 SIDE x REGIME SEGMENTS ===');
  stdout.writeln(
  ...
  stdout.writeln(
    '\nBatch-3 summary: ROBUSTNESS_PASS=$robustnessPassed/${selected.length}, '
